...
void appendToList(UserInfo** head, UserInfo* usr) {
...
UserInfo* getUser(UserInfo* head, string username) {
...
void UserInfoManager::deleteUser(string username) {
    ...
    UserInfo* u = getUser(mylist, username);
...
UserInfo* UserInfoManager::getList() {
    return mylist;
}
...
void HealthAssistant::getBfp(string username) { 
    ...
    // Getting the user from the list
    UserInfo* u = getUser(manager.getList(), username);
    ...
    UserInfo* ptr = getUser(manager.getList(), username);
    ptr->bfp = bfp;
}
